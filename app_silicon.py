...
║  • UI FIX: Decimal precision reduced to 1 for cleaner KPI presentation      ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
with tab2:
    ...
    if "market_price" not in hist.columns or hist["market_price"].isna().all():
        ...
        if len(mkt_overlap) < 3:
            ...
            m1.metric("RMSE (₹/Kg)",  f"₹{rmse:.1f}")
            m2.metric("MAE (₹/Kg)",   f"₹{mae:.1f}")
            m3.metric("MAPE",         f"{mape:.1f}%")
            m4.metric("Correlation",  f"{corr:.3f}")
            m5.metric("Mean Bias",    f"₹{bias:+.1f}/Kg",
            ...
            fig_err.add_trace(go.Bar(
                ...
                hovertemplate="<b>%{x|%d %b %Y}</b><br>Error: ₹%{y:.1f}/Kg<extra></extra>",
            ))
            ...
            fig_err.add_annotation(
                ...
                text=f"Mean Bias: ₹{bias:+.1f}", showarrow=False,
            ...
            fig_hist_err.add_annotation(
                ...
                text=f"Bias={bias:+.1f}", showarrow=False,
            ...
            with st.expander("📋 View Weekly Comparison Data", expanded=False):
                ...
                tbl["Error (₹/Kg)"] = (tbl["Model Price (₹/Kg)"] -
                                        tbl["Market Price (₹/Kg)"]).round(1)
                tbl["Error %"] = (tbl["Error (₹/Kg)"] /
                                   (tbl["Market Price (₹/Kg)"] + 1e-9) * 100).round(1)
                st.dataframe(
