...
# Inject custom CSS to reduce metric sizes and prevent "..." truncation in 1 row
st.markdown("""
<style>
[data-testid="stMetricValue"] {
    font-size: 1.4rem !important; /* Slightly increased value size */
}
[data-testid="stMetricLabel"] {
    font-size: 0.9rem !important; /* Slightly increased label size */
}
[data-testid="stMetricDelta"] {
    font-size: 0.85rem !important; /* Slightly increased delta text size */
}
</style>
...
# Adjust column widths: give more space to columns 3 & 4 which contain large delta tags
...
with tab2:
    ...
    if "market_price" not in hist.columns or hist["market_price"].isna().all():
        ...
        if len(mkt_overlap) < 3:
            ...
            # Create h dataframe before the graph
            h = hist[hist.index >= hist.index[-1] - pd.DateOffset(weeks=display_window)].copy()

            fig_mkt = go.Figure()
            
            # Trace 1: Actual Market
            ...
            # Trace 3: Forecast (Stitched to last history point)
            last_hist_date = h.index[-1]
            last_hist_price = float(h["real_price"].iloc[-1])
