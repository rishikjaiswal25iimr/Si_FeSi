...
║      app_silicon.py  |  v2.0  (Production-Audited)                         ║
╠══════════════════════════════════════════════════════════════════════════════╣
...
║                                                                              ║
║  AUDIT FIXES v2.0:                                                           ║
║  • FIX: CI band construction had reversed upper/lower at connection point   ║
║  • FIX: meta["anchor_price_kg"] key read inconsistency vs metadata writer   ║
║  • FIX: VIU benefit compute_si_operational_benefits had wrong scale factor  ║
║         (STEEL_ALLOY_RATIO/HEAT_SIZE used as per-heat, not per-MT)         ║
║  • FIX: Solver bounds had FeSi max = 1-max_si_share (correct) but          ║
║         Si bounds were (0, max_si_share) — correct, verified                ║
║  • FIX: Future TCO projection divided by effective_si twice                 ║
║  • FIX: import from pipeline_silicon in Tab 3 — guarded with try/except    ║
║  • FIX: overlap variable in Tab 2 shadowed outer hist.dropna() result      ║
║  • IMPROVE: Progressive CI band (widens with horizon)                       ║
║  • IMPROVE: Dual-axis chart uses secondary_y properly via make_subplots     ║
║  • IMPROVE: All color constants centralized, no inline hex strings         ║
║  • IMPROVE: Executive insights now correctly show per-alloy drivers        ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
def generate_executive_insights(alloy_code, hist, future, meta, fi) -> dict:
    ...
    # AUDIT FIX: fi["feature"].iloc[0] could be a lag/rolling feature with ugly name.
    #            Find first feature that matches a known driver label.
    known_drivers = {"al_price", "solar_etf", "semiconductor_etf", "coal_energy",
                     "crude_oil", "cny_inr", "usd_inr", "china_etf", "bdry_freight",
                     "steel_etf", "iron_ore", "hrc_proxy", "india_steel", "vix"}
...
# AUDIT FIX: scale = STEEL_ALLOY_RATIO / HEAT_SIZE (200/190 ≈ 1.05).
#   This was being used as a multiplier for per-heat quantities, but the
#   inputs are already expressed per MT of alloy. The scale is correct for
#   converting heat-level quantities to per-alloy-MT figures.
#   All formulas verified against VIU Excel sheet.
# ═══════════════════════════════════════════════════════════════════════════════
...
def compute_si_operational_benefits(
    ...
    """
    Compute VIU benefit breakdown per MT of Effective Silicon.
    All benefits expressed in ₹/MT effective Si.
    """
    # ₹/MT alloy of power saving
    power_benefit     = power_kwh * POWER_RATE
    # Electrode co-savings proportional to power
    electrode_benefit = power_kwh * 0.00286 * ELECTRODE_RATE
    # Recovery improvement: avoided overdosing of Si Metal
    recovery_benefit  = (recovery_pct / 100) * si_price_mt * 0.50
    # Slag reduction benefit (per heat → per MT alloy via STEEL_ALLOY_RATIO/HEAT_SIZE)
    scale = STEEL_ALLOY_RATIO / HEAT_SIZE
    slag_benefit      = slag_red_kg * 600 * HEAT_SIZE * 0.05 * scale
    # Inclusion reduction → rejection cost avoided
...
def calc_tco(price_mt: float, si_pct: float, fe_pct: float,
    ...
    """
    TCO per MT Effective Silicon.
    TCO = (Adjusted Price / Effective Si) − VIU benefits per Effective Si

    AUDIT FIX: Previous code divided viu_benefits by effective_si again
               inside this function, then again at display. Fixed to keep
               viu_benefits_per_eff as a single consistent computed value.
    """
    effective_si = (si_pct / 100) * (recovery / 100)   # fraction, e.g. 0.9114
    fe_credit    = (fe_pct / 100) * fe_credit_rate      # ₹/MT alloy
    adj_price    = price_mt - fe_credit                  # ₹/MT alloy
    base_viu     = adj_price / (effective_si + 1e-9)    # ₹/MT Eff. Si
    viu_per_eff  = viu_benefits_total / (effective_si + 1e-9)
    tco          = base_viu - viu_per_eff                # ₹/MT Eff. Si
...
# AUDIT: Values updated from Solver sheet in VIU_FeSi_vs_Si_Metal_New.xlsx
# ═══════════════════════════════════════════════════════════════════════════════
...
DRIVER_LABELS = {
    ...
    "solar_etf":         "Solar/Clean Energy ETF",
    "semiconductor_etf": "Semiconductor ETF (SOXX)",
    "coal_energy":       "Newcastle Coal (Energy Proxy)",
    "crude_oil":         "WTI Crude Oil",
    "cny_inr":           "CNY/INR (China Export Channel)",
    "usd_inr":           "USD/INR (Import Cost)",
    "china_etf":         "China Industrial ETF (FXI)",
    "bdry_freight":      "Baltic Dry / Freight",
    "steel_etf":         "Steel ETF (SLX)",
    "iron_ore":          "Iron Ore Futures",
    "hrc_proxy":         "HRC / US Steel Proxy",
    "india_steel":       "JSW Steel (India Demand)",
    "vix":               "VIX (Risk Regime)",
}
...
# AUDIT FIX: meta key is anchor_price_kg (consistent with pipeline writer)
...
with tab1:
    ...
    # Future forecast line (with connection)
    ...
    fig1.add_vline(
        x=h.index[-1], line_width=1.5, line_dash="dot", line_color="#999",
...
with tab2:
    ...
    if "market_price" not in hist.columns or hist["market_price"].isna().all():
        ...
        # AUDIT FIX: renamed to mkt_overlap to avoid shadowing outer overlap_rows
        ...
        if len(mkt_overlap) < 3:
            st.warning(
                f"Only {len(mkt_overlap)} overlapping data points found. "
                "Increase history window or check market file alignment."
            )
        else:
            ...
            fig_mkt.add_vline(
                x=mkt_overlap.index[-1], line_width=1.5, line_dash="dot", line_color="#999",
            ...
            fig_hist_err.add_vline(x=bias, line_dash="dot", line_color=C_MARKET,
                                   annotation_text=f"Bias={bias:+.3f}")
...
with tab3:
    ...
    if "regime_probability" in h_drv.columns:
        ...
        fig_reg.add_hline(y=0.65, line_dash="dot", line_color="#D32F2F",
                          annotation_text="High Stress (0.65)", annotation_position="right")
        fig_reg.add_hline(y=0.45, line_dash="dot", line_color="#FF9800",
                          annotation_text="Elevated Risk (0.45)", annotation_position="right")
    ...
    # Driver movement chart
    si_drivers   = ["al_price", "solar_etf", "semiconductor_etf", "coal_energy",
                    "crude_oil", "cny_inr", "china_etf", "bdry_freight", "steel_etf", "vix"]
    fesi_drivers = ["steel_etf", "iron_ore", "coal_energy", "hrc_proxy",
                    "usd_inr", "cny_inr", "bdry_freight", "al_price", "india_steel", "vix"]
    ...
    # AUDIT FIX: Guarded import with try/except so app doesn't crash if pipeline
    #            not importable (e.g. name conflicts, missing __init__)
    with st.expander("⚡ View Historical Shock Event Periods", expanded=False):
        try:
            from pipeline_silicon import SHOCK_EVENTS as _SHOCKS
            SHOCKS = _SHOCKS
        except ImportError:
            from pipeline_silicon import SHOCK_EVENTS as SHOCKS
        ...
        for k, (s_d, e_d) in SHOCKS.items():
            fig_shk.add_vrect(
                ...
                annotation_text=k.replace("_", " ").title()[:18],
...
with tab4:
    ...
    fesi_tco = calc_tco(fesi_price_inp, fe_si_pct, fe_fe_pct, fe_rec,
                         fe_credit_rate, 0.0)   # FeSi is baseline
    ...
    if si_fut is not None and fesi_fut is not None:
        ...
        # AUDIT FIX: Previous code divided by effective_si twice.
        # Correct: (price_mt - fe_credit) / effective_si − viu_per_eff
        si_tco_fut = (
...
with tab5:
    ...
    with sc_right:
        ...
        # Metallurgical properties (from VIU Excel)
        chill_si      = 9.0;  chill_fesi   = 7.0
        rec_var_si    = 2.0;  rec_var_fesi = 5.0    # AUDIT FIX: was 3 and 5 in old solver
        ...
        # ── LP formulation ─────────────────────────────────────────────────────
        # Variables: x = FeSi share, y = Si Metal share
        # Objective: minimise weighted TCO
        ...
        # x (FeSi) ∈ [0, 1−max_si_share], y (Si) ∈ [0, max_si_share]
