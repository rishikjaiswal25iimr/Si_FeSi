...
with st.sidebar:
    ...
    if comparison_selection != "Not selected":
        # Dynamic Defaults mapped directly from respective Excel logic sheets
        ...
        # MC uses a higher nominal addition rate to hit the active Mn targets
        p1_target_def = 6.7 if is_mc else 5.0
        
        # MC Excel uses simpler 78% efficiency vs LC 45%*60%
        p1_lf_eff_def = 78.0 if is_mc else 45.0
        p1_arc_duty_def = 100.0 if is_mc else 60.0
        ...
        yield_factor_def = 0.000300 if is_mc else 0.000025
        c_corr_freq_def = 0.10
        
        # MC Excel realization factors default to stricter bounds
        r_through_def = 0.25 if is_mc else 0.40
        r_yield_def   = 0.25 if is_mc else 0.20
        r_reblow_def  = 0.50 if is_mc else 0.75
        r_power_def   = 1.00 if is_mc else 0.90
        r_elec_def    = 1.00 if is_mc else 0.90
        r_clean_def   = 1.00 if is_mc else 0.30
        ...
        P_Chill_Alloy1  = st.slider(f"{alloy1_name} Chill Factor (°C/kg/t)", 1.0, 5.0, p1_chill_def, 0.001)
        ...
        P_Alloy_Target = st.number_input("Alloy Addition Rate (kg/T)", value=p1_target_def, step=0.1, min_value=1.0, max_value=20.0, format="%.1f")
        P_LF_Efficiency = st.slider("LF Efficiency (%)",       25.0, 90.0, p1_lf_eff_def, 1.0) / 100
        P_Arc_Duty      = st.slider("Arc Duty Cycle (%)",      30.0, 100.0, p1_arc_duty_def, 1.0) / 100
        ...
        P_Yield_Factor     = st.number_input("Yield Gain Factor",              value=yield_factor_def, format="%.6f")
        ...
        R_Power       = st.slider("Power Realization",       0.50, 1.00, r_power_def, 0.01)
        R_Electrode   = st.slider("Electrode Realization",   0.50, 1.00, r_elec_def, 0.01)
        R_Throughput  = st.slider("Throughput Realization",  0.10, 1.00, r_through_def, 0.01)
        R_Stability   = st.slider("Stability Realization",   0.20, 1.00, 0.50, 0.01)
        R_Reblow      = st.slider("Reblow Realization",      0.30, 1.00, r_reblow_def, 0.01)
        R_Cleanliness = st.slider("Cleanliness Realization", 0.10, 1.00, r_clean_def, 0.01)
        R_Yield       = st.slider("Yield Realization",       0.05, 1.00, r_yield_def, 0.01)
...
# ══════════════════════════════════════════════════════════════════════════════
# CORE CALCULATIONS (RIGOROUS MASS-BALANCE ENGINE)
# ══════════════════════════════════════════════════════════════════════════════

# Calculate true mass balance for substitutions mapping exactly to physical Excel logic
...
# Converts savings mapped per-heat or per-ton-of-steel strictly into ₹/MT of Alternative Alloy
Alt_Heat_Multiplier = 1000.0 / (Alloy_Alt * P_Heat_Size)
Alt_Steel_Multiplier = 1000.0 / Alloy_Alt
...
Power_kWh_Saved_Per_MT = Energy_Saved_per_T_steel * Alt_Steel_Multiplier
...
# --- Throughput Gain ---
Time_Saved_Min_Per_Heat = Delta_Temp_Rigorous / P_Reheat_Rate
Benefit_Throughput = (Time_Saved_Min_Per_Heat / P_Cycle_Time) * P_Heat_Size * P_Margin_Steel * R_Throughput * Alt_Heat_Multiplier

# --- Recovery Stability ---
Cost_Per_Mn_Base_Raw = P_Alloy1_Price / (P_Alloy1_Mn * P_Alloy1_Rec)
Benefit_Stability = (Alloy1_Overdose - Alloy2_Overdose) * Cost_Per_Mn_Base_Raw * R_Stability

# --- Re-treatment Reduction ---
Benefit_Retreatment = (Retreatment_Alloy1 - Retreatment_Alloy2) * P_LF_Retreatment_Cost * R_Reblow * Alt_Heat_Multiplier

# --- Cleanliness ---
Benefit_Cleanliness = (Reject_Alloy1 - Reject_Alloy2) * P_Steel_Value * R_Cleanliness * Alt_Steel_Multiplier

# --- Yield Improvement ---
Benefit_Yield = P_Yield_Factor * P_Steel_Value * R_Yield * Alt_Steel_Multiplier

# --- Carbon Correction Avoidance ---
# Only applied if base alloy triggers correction thresholds (scaled by frequency)
Benefit_Carbon = C_Corr_Freq_Alloy1 * RH_Corr_Time * P_RH_Minute_Cost * Alt_Heat_Multiplier

# --- Hydrogen Penalty ---
Benefit_Hydrogen = -(H2_Bath_Pickup / H2_Degas_Rate) * P_RH_Minute_Cost * Alt_Heat_Multiplier

# --- Refractory Life ---
Benefit_Refractory = (P_Ladle_Reline_Cost / P_Ladle_Life) * Refractory_Wear_Drop * Alt_Heat_Multiplier
...
# Deduct lost Iron Credit as a penalty to get Total Net Credits mapping to Excel
...
with tab1:
    ...
    all_benefit_basis = [
        f"ΔT={Delta_Temp_Rigorous:.3f}°C/heat, {Eff_LF_Effective*100:.0f}% LF eff, {R_Power*100:.0f}% real.",
        f"P_kWh_saved={Power_kWh_Saved_Per_MT:.1f} kWh/MT, {P_Graphite_Factor*1000:.0f}g/kWh, {R_Electrode*100:.0f}% real.",
        f"Time saved={Time_Saved_Min_Per_Heat:.2f} min/heat, {R_Throughput*100:.0f}% real.",
    ...
    with col_chart:
        ...
        fig_bar.update_layout(**_layout(f"Gross Benefit Contribution per MT {alloy2_name} (₹/MT)", "₹/MT Alloy", 460))
...
with tab2:
    ...
    # ── DYNAMIC MATRIX FOR LC vs MC ──
    if is_mc:
        grades_data = {
            "Commodity (IS2062)":   {"c_lim": 0.010, "rec_lim": 0.06,  "inc_lim": 0.12, "metric4_lim": 0.05, "metric5_lim": 0.40, "briq_max": 0.0},
            "TMT/Rebar (Fe500D)":   {"c_lim": 0.008, "rec_lim": 0.05,  "inc_lim": 0.10, "metric4_lim": 0.04, "metric5_lim": 0.30, "briq_max": 0.0},
            "HSLA/API (API X70)":   {"c_lim": 0.005, "rec_lim": 0.03,  "inc_lim": 0.06, "metric4_lim": 0.03, "metric5_lim": 0.20, "briq_max": 1.0},
            "Automotive (DP600)":   {"c_lim": 0.003, "rec_lim": 0.02,  "inc_lim": 0.04, "metric4_lim": 0.02, "metric5_lim": 0.15, "briq_max": 1.0},
